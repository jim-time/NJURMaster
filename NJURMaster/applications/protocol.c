...
#include "stdlib.h"
#include "string.h"
#define caliMaxTime 50
uint16_t can_encoder_flag;
u16 yaw_cali_count = 0;
u16 pitch_cali_count = 0;
...
if(*(data_buf+2)==0X02)
    ...
    {
        PID_arg[0].kp  = 0.01*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );//Chassis_Rotation
        PID_arg[0].ki  = 0.01*( (vs16)(*(data_buf+6)<<8)|*(data_buf+7) );
        PID_arg[0].kd  = 0.01*( (vs16)(*(data_buf+8)<<8)|*(data_buf+9) );
        PID_arg[1].kp = 0.01*( (vs16)(*(data_buf+10)<<8)|*(data_buf+11) );//Chassis_Velocity
        PID_arg[1].ki = 0.01*( (vs16)(*(data_buf+12)<<8)|*(data_buf+13) );
        PID_arg[1].kd = 0.01*( (vs16)(*(data_buf+14)<<8)|*(data_buf+15) );
        PID_arg[2].kp 	= 0.01*( (vs16)(*(data_buf+16)<<8)|*(data_buf+17) );//GimbalPitch_Pos
    ...
    {	
        ...
        //16 17
        FireSpeed = ((vs16)(*(data_buf+16)<<8)|*(data_buf+17));
        //18 19
        CMOutput4 = ((vs16)(*(data_buf+20)<<8)|*(data_buf+21))-1000;
       	if(send_check == 0)
				{
					send_check = 1;
					checkdata_to_send = *(data_buf+2);
...
void RcDataAnalysis(RC_Ctrl_t *rc)
{
	...
	if (IsDeviceLost(DEVICE_INDEX_PC))
	{
		if (GetRcMode()==RC_KEY_RCMODE)
		{
			...
			GimbalPitchPosRef=LIMIT(__temp,PITCH_MIN,PITCH_MAX);
			temp_rc_ch0 = rc->rc.ch0-CHANNELMIDDLE;
			if(my_abs(temp_rc_ch0) < 30)//10
				temp_rc_ch0 = 0;
			__temp=GimbalYawPosRef-temp_rc_ch0*RC_YAWSCALE;
			GimbalYawPosRef=LIMIT(__temp,-YAW_MAX,YAW_MAX);
